...
class _NodeItemStatelessWidget extends StatelessWidget {
  ...
  String _getNodeStatusText() {
    if (aliveStaking.status == 1) {
      return "候选中";
    } else if (aliveStaking.status == 2) {
      return "活跃中";
    } else if (aliveStaking.status == 3) {
      return "出块中";
    } else if (aliveStaking.status == 6) {
      return "共识中";
    }
    return "未知";
  }

  Color _getNodeStatusColor() {
    if (aliveStaking.status == 1) {
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                width: 1, color: _getNodeStatusColor())),
                        child: Text(
                          _getNodeStatusText(),
                          style: TextStyle(
                              color: _getNodeStatusColor(), fontSize: 10),
                        ),
                      ),
                    ],
                  ),
