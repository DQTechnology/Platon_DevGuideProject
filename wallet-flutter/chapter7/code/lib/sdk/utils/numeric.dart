...
class Numeric {
  ...
  static Uint8List toBytesPadded(BigInt value, int length) {
    Uint8List result = Uint8List(length);
    Uint8List bytes = unsignedIntToBytes(value);
    int bytesLength;
    int srcOffset;
    if (bytes[0] == 0) {
    ...
    if (bytesLength > length) {
      throw  Exception("Input is too large to put in byte array of size $length");
    }

    int destOffset = length - bytesLength;

    List.copyRange(result, destOffset, bytes, srcOffset);
    return result;
  }
