...
enum class TransactionType(val txTypeValue: Int, val txTypeDescRes: Int) {
    /**
     * 未知
     */
    ...
    companion object {
        private val map: MutableMap<Int, TransactionType> = HashMap()

        fun getTxTypeByValue(value: Int): TransactionType {
            val transactionType = map[value]
            return transactionType ?: UNKNOWN
        }

        init {
            for (status in TransactionType.values()) {
                map[status.txTypeValue] = status
            }
        }
    }
