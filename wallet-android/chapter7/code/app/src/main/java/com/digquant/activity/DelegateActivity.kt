...
class DelegateActivity : BaseActivity() {
    ...
    override fun initUI() {
        ...
        binding.delegateWalletName.text = WalletManager.GetCurrentWalletName()
        binding.delegateWalletAddress.text =
            AddressFormatUtil.formatAddress(WalletManager.GetCurWalletAddress())


        lifecycleScope.launch {
            val balance = PlatonApi.GetBalance(WalletManager.GetCurWalletAddress())

            binding.balance.text = balance.toString()

            binding.delegateFee.text = AmountUtil.convertVonToLat(
                PlatonApi.GetGasPrice().multiply(BigInteger("21000")).toString()
            )
        }
    }
