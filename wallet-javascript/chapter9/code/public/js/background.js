...
class NetworkManager {
    ...
    static GetChainId() {
        let networkInfo = NetworkManager.networkInfos[NetworkManager.curNetwork];
        return parseInt(networkInfo.chainId);
    }
}
...
class DelegateManager {
    ...
    static async GetDelegateInfoList() {
        let web3 = NetworkManager.GetWebIns();
        let curAccountInfo = await PrivateKeyManager.GetAccountInfo();
        if (!curAccountInfo) {
            return {
                errCode: ERROR,
                errMsg: "当前未选择账号"
        ...
        let hexAddress = web3.utils.decodeBech32Address(curAccountInfo.address);
        let ppos = new web3.PPOS({
            provider: NetworkManager.GetCurNetworkRPCUrl(),
            chainId: NetworkManager.GetChainId()
        });
        let hexStrBufAddr = ppos.hexStrBuf(hexAddress);

        // 获取委托的所有节点
        let delegateNodeListRes = await DelegateManager.getRelatedListByDelAddr(
            ppos,
            hexStrBufAddr
        );

        let delegateNodeList = delegateNodeListRes.data;
        //没有委托
        if (!delegateNodeList) {
            return {
                delegationList: [],
        ...
        let delegationList = [];
        let totalDelegateBN = null;
        let totalRewardBN = null;
        // 获取所有节点待领取的奖励
        let rewardMap = await DelegateManager.getRewardMap(ppos, hexStrBufAddr);

        for (let i = 0; i < delegateNodeList.length; ++i) {
            let delegateNode = delegateNodeList[i];

            let nodeStrBufId = ppos.hexStrBuf(delegateNode.NodeId);

            let nodeInfo = await DelegateManager.getNodeInfo(ppos, nodeStrBufId);
            if (!nodeInfo) {
                continue;
            }
            // 获取节点的委托信息
            let delegateInfo = await DelegateManager.getSingleDelegateInfo(
                ppos,
                hexStrBufAddr,
                nodeStrBufId,
                delegateNode
            ...
            // 自由资金
            let releaseLAT = web3.utils.hexToNumberString(delegateInfo.Released);
            // 锁仓资金
            let restrictingPlanLAT = web3.utils.hexToNumberString(delegateInfo.RestrictingPlan);

            let delegateLATBN = web3.utils
                .toBN(releaseLAT)
                .add(web3.utils.toBN(restrictingPlanLAT));

            if (totalDelegateBN) {
                totalDelegateBN.add(delegateLATBN);
            } else {
                totalDelegateBN = delegateLATBN;
            }

            let reward = rewardMap[delegateNode.NodeId];

            if (totalRewardBN) {
                totalRewardBN.add(web3.utils.toBN(reward));
            } else {
                totalRewardBN = web3.utils.toBN(reward);
            }

            let deletegateLAT = web3.utils.fromVon(
                delegateLATBN.toString(), // 这里只显示自由资金
                "lat"
            );

            delegationList.push({
                nodeId: delegateNode.NodeId, // 节点id
                stakingBlockNum: delegateNode.StakingBlockNum,
                nodeName: nodeInfo.NodeName, // 节点名
                deletegateLAT: deletegateLAT, // 委托LAT数量
                reward: web3.utils.fromVon(reward) // 待领取的奖励
            });
        }

        let totalReward = 0;
        if (totalRewardBN) {
            totalReward = web3.utils.fromVon(totalRewardBN.toString());
        }
        let totalDelegate = 0;
        if (totalDelegateBN) {
            totalDelegate = web3.utils.fromVon(totalDelegateBN.toString());
        }
    ...
    static async getSingleDelegateInfo(ppos, hexStrBufAddr, nodeStrBufId, delegateNode) {
        let params = {
            funcType: 1104, // 委托LAT的函数
            delAddr: hexStrBufAddr,
            stakingBlockNum: delegateNode.StakingBlockNum,
            nodeId: nodeStrBufId
        };
        let reply = await ppos.call(params);
        if (reply.Code !== 0) {
            return null;
        }
        return reply.Ret;
    ...
    static async getRewardMap(ppos, hexStrBufAddr) {
        let params = {
            funcType: 5100, // 委托LAT的函数
            address: hexStrBufAddr,
            nodeIDs: [] // 为空则查询所有节点
        };
        ...
        let web3 = NetworkManager.GetWebIns();
        // rewardMap
        for (let i = 0; i < reply.Ret.length; ++i) {
            let rewardInfo = reply.Ret[i];

            rewardMap[rewardInfo.nodeID] = web3.utils.hexToNumberString(rewardInfo.reward);
        }
